...
function findCorrLength!(lengths::Vector{Float64},n::Int,β::Float64,μ::Float64,tmpGraphs::Vector{disjointSetGraph},tmpNews::Vector{Vector{Float64}},edges::Vector{Vector{Vector{Int}}},otherCords::Vector{Vector{Int}},ps::Vector{Float64},cords)
    ps[1]=1-1/(1+exp(β*(μ)))
...
open("outb_"*string(n)*"_"*string(k)*"_"*pointfile,"w") do io#Create file if it doesn't exist, otherwise overrite existing dfile of same name.
    nothing
end
open("outb_"*string(n)*"_"*string(k)*"_"*pointfile,"a") do io #Open file for appending
    t=@time @showprogress 1 "Computing..." for i=eachindex(lengths)
        findCorrLength!(lengths[i],n,points[i,1],points[i,2],tmpGraphs,tmpNews,edges,otherCords,ps,cords)
